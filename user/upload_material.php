...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title === '' || $subject_id === 0) {
        $errors[] = "Please provide both material title and select a subject.";
    }
  ...
  <div class="col-lg-8">
    <div class="card card-sm p-4 p-md-5">
      <div class="d-flex align-items-center gap-3 mb-4 border-bottom pb-3">
        <div class="d-inline-flex align-items-center justify-content-center bg-success bg-opacity-10 text-success rounded-3 p-3">
          <i class="fa-solid fa-cloud-arrow-up fa-2xl"></i>
        </div>
        <div>
          <h4 class="fw-bold mb-1">Share Academic Resource</h4>
          <p class="text-muted small mb-0">Help fellow NSBM students by contributing lecture notes, past papers, or tutorial solutions</p>
        </div>
      </div>
      ...
      <form method="POST" enctype="multipart/form-data" id="uploadForm" novalidate>
        <div class="mb-3">
          ...
          <input type="text" name="title" class="form-control" placeholder="e.g. Programming Fundamentals 2025 Mid-Semester Past Paper" required value="<?php echo htmlspecialchars($_POST['title'] ?? ''); ?>">
        </div>
        ...
        <div class="mb-3">
          ...
          <textarea name="description" class="form-control" rows="3" placeholder="Provide context about this material (e.g. key topics covered, exam batch, or lecturer references)..."><?php echo htmlspecialchars($_POST['description'] ?? ''); ?></textarea>
        </div>
        ...
        <div class="mb-4">
          ...
          <div class="form-text mt-2 text-muted small">
            <i class="fa-solid fa-circle-info me-1"></i> Supported formats: <span class="badge bg-light text-dark border">PDF</span> <span class="badge bg-light text-dark border">DOCX</span> <span class="badge bg-light text-dark border">PPTX</span> <span class="badge bg-light text-dark border">ZIP</span> (Max 20MB)
          </div>
        </div>

        <div class="d-flex align-items-center justify-content-between pt-2">
          <a href="materials.php" class="btn btn-outline-secondary rounded-pill px-4">Cancel</a>
          <button type="submit" class="btn btn-sm-primary rounded-pill px-4"><i class="fa-solid fa-paper-plane me-2"></i>Submit for Review</button>
        </div>
      </form>
    </div>
  </div>
